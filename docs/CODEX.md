...
Career-Copilot supports Codex through the root `AGENTS.md` file.
...
- `Evaluate this job URL with Career-Copilot and run the full pipeline.`
- `Scan my configured portals for new roles that match my profile.`
- `Generate the tailored ATS PDF for this role using Career-Copilot.`
...
Career-Copilot modes and scripts rather than introducing a parallel automation
layer.
...
- On no-login platforms (Greenhouse, Lever), auto-submit applications and flag the user afterward. For login-required platforms, prepare everything and let the user submit.
