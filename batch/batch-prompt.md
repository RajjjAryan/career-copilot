...
2. If the file is empty or doesn't exist, try to get the JD from `{{URL}}` with `web_fetch`
...
Read `cv.md`. Table with each JD requirement mapped to exact lines from the CV.
...
3. **"If downleveled" plan**: accept if fair comp, 6-month review, clear criteria

#### Block D — Comp and Demand

Use `web_search` for current salaries (Glassdoor, Levels.fyi, Blind), company comp reputation, demand trends. Table with data and cited sources. If no data available, say so.
