...
for (const { path, name } of filesToCheck) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (matches) {
      warnings.push(`${name}:${i + 1} — Possible hardcoded metric: "${matches[0]}". Should this be read from cv.md?`);
    }
  }
}

// 4. article-digest.md freshness
...
if (existsSync(digestPath)) {
  ...
  if (daysSinceModified > 30) {
    warnings.push(`article-digest.md is ${Math.round(daysSinceModified)} days old. Consider updating.`);
  }
}
