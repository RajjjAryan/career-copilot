 ...
 * These are NOT native <select> elements — they use role="combobox"
 * with .select__input / .select__option classes.
 */
async function selectReactOption(page, question, answerValue) {
  ...
  try {
    // Find the React-Select input for this question's container
    // Questions are rendered in containers with question_{id} pattern
    const questionId = question.id || '';
    const container = page.locator(`[id*="${questionId}"]`).first();
    if (await container.count() === 0) return;

    // Find the combobox input within or near the container
    const combobox = page.locator(`#question_${questionId}_input, [id*="${questionId}"] .select__input input, [id*="${questionId}"][role="combobox"]`).first();
    if (await combobox.count() === 0) {
      // Fallback: try clicking the container's select control
      const selectControl = page.locator(`[id*="${questionId}"] .select__control`).first();
      if (await selectControl.count() > 0) {
        await selectControl.click();
        await page.waitForTimeout(400);
        // Click the matching option using .select__option (avoids phone picker)
    ...
    await combobox.fill(labelText.substring(0, 20));
    await page.waitForTimeout(500);

    // Use .select__option specifically (NOT generic [role="option"])
    const opt = page.locator(`.select__option:has-text("${labelText.substring(0, 40)}")`).first();
